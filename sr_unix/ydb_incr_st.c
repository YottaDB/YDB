		...
		const ydb_buffer_t *increment, ydb_buffer_t *ret_value)
{
	...
	threaded_api_ydb_engine_lock(tptoken, errstr, LYDB_RTN_INCR, &save_active_stapi_rtn, &save_errstr, &get_lock, &retval);
	if (YDB_OK == retval)
	{
		caller_func_is_stapi = TRUE;	/* used to inform below SimpleAPI call that caller is SimpleThreadAPI */
		retval = ydb_incr_s(varname, subs_used, subsarray, increment, ret_value);
