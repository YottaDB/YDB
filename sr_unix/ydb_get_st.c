		...
		ydb_buffer_t *ret_value)
{
	...
	THREADED_API_YDB_ENGINE_LOCK(tptoken, errstr, LYDB_RTN_GET, save_active_stapi_rtn, save_errstr, get_lock, retval);
	if (YDB_OK == retval)
	{
		caller_func_is_stapi = TRUE;	/* used to inform below SimpleAPI call that caller is SimpleThreadAPI */
		retval = ydb_get_s(varname, subs_used, subsarray, ret_value);
