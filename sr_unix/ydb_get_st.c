 ...
 *
 * Parms and return - same as ydb_get_s() except for the addition of tptoken.
 */
int ydb_get_st(uint64_t tptoken, ydb_buffer_t *varname, int subs_used, ydb_buffer_t *subsarray, ydb_buffer_t *ret_value)
{
	...
	retval = ydb_stm_args4(tptoken, LYDB_RTN_GET, (uintptr_t)varname, (uintptr_t)subs_used, (uintptr_t)subsarray,
			       (uintptr_t)ret_value);
	return (int)retval;
}
