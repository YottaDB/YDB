...
int4 mupip_set_file(int db_fn_len, char *db_fn)
{
	...
	sd = (sgmnt_data *)malloc(ROUND_UP(sizeof(sgmnt_data), DISK_BLOCK_SIZE));
	...
	for (;  rptr != NULL;  rptr = rptr->fPtr)
	{
		...
		access_new = (n_dba == access ? sd->acc_meth : access);
							/* recalculate; n_dba is a proxy for no change */
		change_fhead_timer("FLUSH_TIME", sd->flush_time,
				   (dba_bg == access_new ? TIM_FLU_MOD_BG : TIM_FLU_MOD_MM),
				   FALSE);
		if ((n_dba != access) && (sd->acc_meth != access))	/* n_dba is a proxy for no change */
		{
			if (dba_mm == access)
				sd->defer_time = 1;			/* defer defaults to 1 */
			sd->acc_meth = access;
			if (0 == sd->n_bts)
			{
				sd->n_bts = WC_DEF_BUFFS;
				sd->bt_buckets = getprime(sd->n_bts);
			}
		}
		if (glbl_buff_status)
		{
			sd->n_bts = BT_FACTOR(new_cache_size);
			sd->bt_buckets = getprime(sd->n_bts);
			sd->n_wrt_per_flu = 7;
			sd->flush_trigger = FLUSH_FACTOR(sd->n_bts);
		}
		if (disk_wait_status)
			sd->wait_disk_space = new_disk_wait;
		...
		if(bypass_partial_recov)
		{
			...
			util_out_print("Database file !AD now has partial recovery flag set to  !UL(FALSE) ",
				TRUE, fn_len, fn, sd->file_corrupt);
		}
		...
		LSEEKWRITE(fd, 0, sd, sizeof(sgmnt_data), status);
		if (0 != status)
		{
			save_errno = errno;
			errptr = (char *)STRERROR(save_errno);
	...
	return(exit_stat & EXIT_ERR ? (int4)ERR_WCERRNOTCHG :
	      (exit_stat & EXIT_WRN ? (int4)ERR_WCWRNNOTCHG :
	      (exit_stat & EXIT_INF ? (int4)ERR_WCINFNOTCHG : SS_NORMAL)));
}
