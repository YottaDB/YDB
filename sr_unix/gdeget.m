...
; header
	...
	if (v6311=1) new SIZEOF do v6312init
	s v6311=0
	s v6310=0
...
; templates
	...
	if (gldfmt>10) d
	. d tmpreg("AUTODB")
	. s tmpreg("STATS")='(tmpreg("AUTODB")\2#2)
	. s tmpreg("AUTODB")=tmpreg("AUTODB")#2
	. s:gldfmt>15 tmpreg("AUTODELETE")=(tmpreg("AUTODB")\8)#2
...
region:
	...
	if (gldfmt>14) s regs(s,"BUFFER_SIZE")=$$bin2num($zextract(rec,rel,rel+3)),rel=rel+4
	if (gldfmt<=14) s regs(s,"BUFFER_SIZE")=$$bin2num($ze(rec,rel,rel+1)),rel=rel+2
	...
	s regs(s,"AUTODB")=0,regs(s,"STATS")=1,regs(s,"AUTODELETE")=0,regs(s,"LOCK_CRIT")=0
	i (gldfmt>9) do
	. s regs(s,"EPOCHTAPER")=$$bin2num($ze(rec,rel)),rel=rel+1
	. i (gldfmt>10) do
	. . s x=$$bin2num($ze(rec,rel)),regs(s,"AUTODB")=x#2,regs(s,"STATS")='(x\2#2)
	. . s:gldfmt>15 regs(s,"AUTODELETE")=(x\8#2)
	. . s rel=rel+1
