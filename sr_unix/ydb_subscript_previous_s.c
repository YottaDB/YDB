...
int ydb_subscript_previous_s(const ydb_buffer_t *varname, int subs_used, const ydb_buffer_t *subsarray, ydb_buffer_t *ret_value)
{
	...
	/* Do some validation */
	VALIDATE_VARNAME(varname, subs_used, FALSE, LYDB_RTN_SUBSCRIPT_PREVIOUS, -1, get_type, get_svn_index);
	if (NULL == ret_value)
		...
		case LYDB_VARREF_LOCAL:
			...
			{	/* We have some subscripts - subcases are as follows:
				...
				{	/* Base local variable does not exist.
					 * Return YDB_ERR_NODEEND (i.e. no more subscripts) for "ydb_subscript_previous_s" result.
					 */
					previoussub.mvtype = MV_STR;
					previoussub.str.len = 0; /* this lets us return YDB_ERR_NODEEND when we fall through */
					break;
				}
