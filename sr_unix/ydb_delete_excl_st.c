 ...
 *
 * Parms and return - same as ydb_delete_excl_s() except for the addition of tptoken.
 */
int ydb_delete_excl_st(uint64_t tptoken, int namecount, ydb_buffer_t *varnames)
{
	...
	retval = ydb_stm_args2(tptoken, LYDB_RTN_DELETE_EXCL, (uintptr_t)namecount, (uintptr_t)varnames);
	return (int)retval;
}
