...
boolean_t mu_rndwn_repl_instance(replpool_identifier *replpool_id, boolean_t immediate, boolean_t rndwn_both_pools)
{
	...
	memset(shmid_buff, 0, TMP_BUF_LEN);
	...
	{	/* --------------------------
		...
		if (SS_NORMAL == (jnlpool_stat = mu_replpool_grab_sem(&repl_instance, JNLPOOL_SEGMENT, &sem_created)))
		{
			...
			if (INVALID_SHMID != shm_id)
			{
				...
				i2asc((uchar_ptr_t)shmid_buff, shm_id);
			...
			{	/* Invoked by MUPIP RUNDOWN in which case the semaphores needs to be removed. But, remove the
				...
				if ((sem_created || (SS_NORMAL == jnlpool_stat))
					&& (SS_NORMAL == mu_replpool_remove_sem(&repl_instance, JNLPOOL_SEGMENT, TRUE)))
				{	/* Now that semaphores are removed, reset fields in file header */
					...
					{	/* if sem_id was "created" by mu_replpool_grab_sem then do NOT report the
						...
						i2asc((uchar_ptr_t)shmid_buff, sem_id);
						gtm_putmsg(VARLSTCNT(9) ERR_MUJPOOLRNDWNSUC, 4, LEN_AND_STR(shmid_buff),
							LEN_AND_STR(replpool_id->instfilename), ERR_SEMREMOVED, 1, sem_id);
					}
				 ...
				 */
				assert((sem_created || (SS_NORMAL == jnlpool_stat)) || holds_sem[SOURCE][JNL_POOL_ACCESS_SEM]);
		...
		} else if (rndwn_both_pools && (INVALID_SHMID != shm_id))
		{
			i2asc((uchar_ptr_t)shmid_buff, shm_id);
			if (rndwn_both_pools)
				gtm_putmsg(VARLSTCNT(6) ERR_MUJPOOLRNDWNFL, 4, LEN_AND_STR(shmid_buff),
	...
	{	/* --------------------------
		...
		if (SS_NORMAL == (recvpool_stat = mu_replpool_grab_sem(&repl_instance, RECVPOOL_SEGMENT, &sem_created)))
		{
			...
			{	/* Invoked by MUPIP RUNDOWN in which case the semaphores needs to be removed. But, remove the
				...
				if ((sem_created || (SS_NORMAL == recvpool_stat))
					&& (SS_NORMAL == mu_replpool_remove_sem(&repl_instance, RECVPOOL_SEGMENT, TRUE)))
				{	/* Now that semaphores are removed, reset fields in file header */
					if (!sem_created)
					{	/* if sem_id was "created" by mu_replpool_grab_sem then do NOT report the
						 * MURPOOLRNDWNSUC message as it indicates that the semaphore was orphaned and we
		...
		} else if (rndwn_both_pools && (INVALID_SHMID != shm_id))
		{
			i2asc((uchar_ptr_t)shmid_buff, shm_id);
			if (rndwn_both_pools)
				gtm_putmsg(VARLSTCNT(6) ERR_MURPOOLRNDWNFL, 4, LEN_AND_STR(shmid_buff),
					LEN_AND_STR(replpool_id->instfilename));
		}
	}
	assert(jgbl.onlnrlbk || (NULL == jnlpool.repl_inst_filehdr));
	if (mur_options.rollback && (SS_NORMAL == jnlpool_stat) && (SS_NORMAL == recvpool_stat))
	{
		assert(jgbl.onlnrlbk || ((INVALID_SHMID == repl_instance.jnlpool_shmid)
	...
	} else /* for MUPIP RUNDOWN, semid fields in the file header are reset and is written in mu_replpool_remove_sem() above */
		decr_cnt = TRUE;
	REVERT;
