 ...
 *	original code.
 *
 *      Note also this module is a near copy of op_fnzpiece.c
 *	differing only in that it calls matchc() instead of
 *	matchb() to do matching.
 * ----------------------------------------------------------
 */
...
void op_setpiece(mval *src, mval *del, mval *expr, int4 first, int4 last, mval *dst)
{
	...
	/* Valid delimiter -  See if we can take a short cut to op_fnp1. If so, the delimiter value needs to be reformated */
