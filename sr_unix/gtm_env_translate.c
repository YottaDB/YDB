...
mval* gtm_env_translate(mval* val1, mval* val2, mval* val_xlated)
{
	...
	MIDENT_CONST(routine_name, GTM_ENV_XLATE_ROUTINE_NAME);
	...
	if (0 != env_gtm_env_xlate.len)
	{
		...
		if (MAX_DBSTRLEN < val_xlated->str.len)
			RTS_ERROR_CSA_ABT(NULL, VARLSTCNT(4) ERR_XTRNRETVAL, 2, val_xlated->str.len, MAX_DBSTRLEN);
		if (0 != ret_gtm_env_xlate)
		{
			if ((val_xlated->str.len) && (val_xlated->str.addr))
				RTS_ERROR_CSA_ABT(NULL, VARLSTCNT(6) ERR_XTRNTRANSERR, 0, ERR_TEXT,  2,
					val_xlated->str.len, val_xlated->str.addr);
			else
				RTS_ERROR_CSA_ABT(NULL, VARLSTCNT(1) ERR_XTRNTRANSERR);
		}
		if ((NULL == val_xlated->str.addr) && (0 != val_xlated->str.len))
			RTS_ERROR_CSA_ABT(NULL, VARLSTCNT(1) ERR_XTRNRETSTR);
