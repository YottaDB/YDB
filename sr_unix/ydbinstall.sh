...
help_exit()
{
	...
	echo "--source-build-dir		-> compiles YottaDB in a specific directory, not /tmp; use with --from-source"
        ...
        echo ""
        echo "Example usages are (assumes latest YottaDB release is r2.04 and latest GT.M version is V7.1-010)"
        echo "  $0                              # installs latest YottaDB release (r2.04) at /usr/local/lib/yottadb/r204"
        echo "  $0 --utf8                       # installs YottaDB release r2.04 with added support for UTF-8"
        echo "  $0 --installdir /r202 r2.02     # installs YottaDB r2.02 at /r202"
        echo "  $0 --gtm                        # installs latest GT.M version (V7.1-010) at /usr/local/lib/fis-gtm/V7.1-010_x86_64"
        echo "  $0 --from-source --branch r2.04 # builds YottaDB r2.04 and installs it at /usr/local/lib/yottadb/r204"
        echo ""
...
if [ -n "$gtm_linkexec" ] ; then
	...
	( cd $gtm_linkexec ; \rm -f ydb gtm ydbsh yottadb ; ln -s $ydb_installdir/ydb $ydb_installdir/gtm $ydb_installdir/ydbsh $ydb_installdir/yottadb ./ )
