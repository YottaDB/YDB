...
STATICFNDCL void cleanup_trigger_hash(char *trigvn, int trigvn_len, char **values, unsigned short *value_len, uint4 big_hash,
				      uint4 little_hash, boolean_t del_little_hash, int match_index);
STATICFNDCL void cleanup_trigger_name(char *trigvn, int trigvn_len, char *trigger_name, int trigger_name_len);
STATICFNDCL int4 update_trigger_hash_value(char *trigvn, int trigvn_len, char **values, unsigned short *value_len, uint4 big_hash,
					   uint4 little_hash, int old_trig_index, int new_trig_index);
