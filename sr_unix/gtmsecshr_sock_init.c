...
#						endif
		...
		if (!ret_status)
		{
			...
			/* If ret_status is zero do the following checks if $gtm_dist/libyottadb.so is not world accessible
			 * then set mode to 0660 and change the gid to the gid of $gtm_dist/libyottadb.so if different from
			 * current user.
			 */
			if (!ret_status)
			{
