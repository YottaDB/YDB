...
boolean_t trigger_name_search(char *trigger_name, uint4 trigger_name_len, mval *val, gd_region **srch_reg)
{
	...
	gvnh_reg_t		*gvnh_reg;
	int			len;
	unmanaged_mname_entry	gvname; /* Will point to gvname_buf */
	sgm_info		*save_sgm_info_ptr;
	jnlpool_addrs_ptr_t	save_jnlpool;
	char			gvname_buf[MAX_MIDENT_LEN];
	...
	{	/* if SET is not found OR KILL is specified in commands, test for KILL hash match */
		...
		if (kill_found)
		{
			if (!set_found || (kill_indx != set_indx))
			{
				...
				{
					setname->umval = killname->umval;
					set_indx = kill_indx;
				}
			} else
				killname->umval = setname->umval;
		} else
		{
			kill_indx = -1;
			if (!set_found)
		...
		int4 *record_num)
{
	...
	unmanaged_mname_entry	gvname; /* Will point to trigvn buffer */
	int4			max_len;
	...
	if (multi_line_xecute)
	{
		if (NULL != trigfile_device)
		{
			...
			multi_jrec.umval = trigger_rec->umval;
			multi_jrec.str.in_array = FALSE;
			trigjrec = &multi_jrec;
	...
	if (0 == dollar_tlevel)
	{
		...
		for (loopcnt = 0; ; loopcnt++)
		{
			...
			if (cdb_sc_onln_rlbk2 == failure)
				RTS_ERROR_CSA_ABT(gv_target ? (gv_target->gd_csa ? gv_target->gd_csa : NULL ) : NULL,
					VARLSTCNT(1) ERR_DBROLLEDBACK);
