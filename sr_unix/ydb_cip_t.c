 ...
 *
 * Parms and return - same as ydb_cip() except for the addition of tptoken.
 */
int ydb_cip_t(uint64_t tptoken, ci_name_descriptor *ci_info, ...)
{
	va_list		var;
	intptr_t	retval;
	...
	retval = ydb_stm_args2(tptoken, LYDB_RTN_YDB_CIP, (uintptr_t)ci_info, (uintptr_t)&var);
	return (int)retval;
}
