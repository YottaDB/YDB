				...
				const ydb_buffer_t *subsarray, ydb_buffer_t *ret_value)
{
	...
	threaded_api_ydb_engine_lock(tptoken, errstr, LYDB_RTN_SUBSCRIPT_PREVIOUS, &save_active_stapi_rtn, &save_errstr, &get_lock,
				     &retval);
	if (YDB_OK == retval)
	{
		caller_func_is_stapi = TRUE;	/* used to inform below SimpleAPI call that caller is SimpleThreadAPI */
		retval = ydb_subscript_previous_s(varname, subs_used, subsarray, ret_value);
