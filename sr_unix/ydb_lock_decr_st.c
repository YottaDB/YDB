 ...
 *
 * Parms and return - same as ydb_lock_decr_s() except for the addition of tptoken.
 */
int ydb_lock_decr_st(uint64_t tptoken, ydb_buffer_t *varname, int subs_used, ydb_buffer_t *subsarray)
{
	...
	retval = ydb_stm_args3(tptoken, LYDB_RTN_LOCK_DECR, (uintptr_t)varname, (uintptr_t)subs_used,
			       (uintptr_t)subsarray);
	return (int)retval;
}
