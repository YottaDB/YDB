 ...
 *
 * Parms and return - same as ydb_node_next_s() except for the addition of tptoken and errstr.
 */
			...
			int *ret_subs_used, ydb_buffer_t *ret_subsarray)
{
	...
	THREADED_API_YDB_ENGINE_LOCK(tptoken, errstr, LYDB_RTN_NODE_NEXT, save_active_stapi_rtn, save_errstr, get_lock, retval);
	if (YDB_OK == retval)
	{
