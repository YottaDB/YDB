 ...
 */
int gtcm_shmclean_main(int argc, char_ptr_t argv[], char **envp)
{
	...
	GTM_THREADGBL_INIT;
	/* This executable does not have any command tables so initialize command array (2nd parm to "common_startup_init") to NULL.
	 * The reason why "cmd_ary" is needed is because trans_log_name (invoked by this module) in turn pulls in
	 * gtm_malloc/gtm_free and in turn a lot of the database runtime logic which in turn (due to triggers)
	 * pulls in the compiler as well (op_zcompile etc. require cmd_ary).
	 */
	common_startup_init(GTCM_SHMCLEAN_IMAGE, NULL);
	err = 0;
