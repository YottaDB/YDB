...
LITDEF	err_msg merrors[] = {
	...
	{ "GTMCHECK", "Internal GT.M error--Report to your GT.M Support Channel", 0, 0 },
	...
	{ "MUPIPINFO", "!AD", 2, 0 },
	{ "IVTIME", "Invalid time specification: !AD", 2, 0 },
	...
	{ "ZPRIVARGBAD", "!AD is not a legal privilege for $ZPRIV()", 2, 0 },
	{ "ZPRIVSYNTAXERR", "Privilege string cannot end with a comma", 0, 0 },
	...
	{ "ZSETPRVARGBAD", "!AD is not a legal privilege for $ZSETPRIV()", 2, 0 },
	{ "INVSPECREC", "Invalid global modifier record", 0, 0 },
	{ "ZSETPRVSYNTAX", "$ZSETPRIV() privileges string cannot end with a comma", 0, 0 },
	...
	{ "ZCSTATUS", "External call: Unsuccessful return status", 0, 0 },
	{ "ZCUSRRTN", "External call: Run-time error in user routine", 0, 0 },
	...
	{ "WILLEXPIRE", "This copy of GT.M will expire within one week", 0, 0 },
	...
	{ "NOTEXTRINSIC", "Quit does not return to an extrinsic function: argument not allowed", 0, 0 },
	...
	{ "REQRUNDOWN", "Error accessing database !AD. Run MUPIP RUNDOWN on node !AD", 4, 0 },
	...
	{ "LINKVERSION", "This image must be relinked with the current version of GT.M", 0, 0 },
	...
	{ "ZLKIDBADARG", "The tvexpr must be FALSE if last ZLKID not found", 0, 0 },
	{ "DBFILOPERR", "Error doing database I/O to region !AD", 2, 0 },
	...
	{ "VMSMEMORY", "Central memory exhausted during request for !UL bytes from 0x!XJ - check page file quota and page file size", 2, 0 },
	...
	{ "BIGNOACL", "Existing file found when BIGRECORD specified with UDF format but no GT.M ACE, perhaps lost during COPY", 0, 0 },
	...
	{ "GVINCRISOLATION", "$INCREMENT cannot be performed on global ^!AD as it has NOISOLATION turned ON", 2, 0 },
	...
	{ "DISTPATHMAX", "$gtm_dist path is greater than maximum (!UL)", 1, 0 },
	{ "FILEOPENFAIL", "Failed to open file !AD", 2, 0 },
	{ "IMAGENAME", "The executing module name should be !AD instead of !AD", 4, 0 },
	{ "GTMSECSHRPERM", "The gtmsecshr module in $gtm_dist (!AD) does not have the correct permission: !AD, and UID: !UL", 5, 0 },
	{ "GTMDISTUNDEF", "Environment variable $gtm_dist is not defined", 0, 0 },
	...
	{ "DBINCRVER", "!AD Incorrect version of GT.M database", 2, 1 },
	...
	{ "DBTNLTCTN", "Transaction numbers greater than the current transaction were found", 0, 5 },
	...
	{ "EXTRIOERR", "Error writing extract file !AD", 2, 0 },
	{ "EXTRCLOSEERR", "Error closing extract file !AD", 2, 0 },
	...
	{ "REPLINSTUNDEF", "Replication instance environment variable $gtm_repl_instance is undefined", 0, 0 },
	...
	{ "REPLREQRUNDOWN", "Error accessing replication instance !AD. Run MUPIP RUNDOWN -REGION \"*\" on node !AD", 4, 0 },
	...
	{ "REQRECOV", "Error accessing database !AD. Run MUPIP JOURNAL -RECOVER on node !AD", 4, 0 },
	...
	{ "DLLVERSION", "Routine !AD in library !AD was compiled with an incompatible version of GT.M.  Recompile with the current version and re-link.", 4, 0 },
	...
	{ "REPLREQROLLBACK", "Replication instance file !AD indicates abnormal shutdown or an incomplete ROLLBACK. Run MUPIP JOURNAL -ROLLBACK", 2, 0 },
	{ "REQROLLBACK", "Error accessing database !AD. Run MUPIP JOURNAL -ROLLBACK -NOONLINE on node !AD", 4, 0 },
	...
	{ "GTMERREXIT", "GTM image has exited with errors", 0, 0 },
	...
	{ "ICUSYMNOTFOUND", "Symbol !AD not found in the ICU libraries. ICU needs to be built with symbol-renaming disabled or gtm_icu_version environment variable needs to be properly specified", 2, 0 },
	{ "STUCKACT", "Process stuck script invoked: !AD : !AD", 4, 0 },
	{ "CALLINAFTERXIT", "After a gtm_exit, a process can never create a valid GT.M context", 0, 0 },
	...
	{ "INVTRCGRP", "Invalid trace group specified in $gtm_trace_groups: !AD", 2, 0 },
	...
	{ "REPL2OLD", "Instance !AD uses a GT.M version that does not support connection with the current version on instance !AD", 4, 0 },
	...
	{ "CUSTERRNOTFND", "Error mnemonic !AD specified in custom errors file is not valid for this version of GT.M", 2, 0 },
	...
	{ "GTMSECSHRBADDIR", "gtmsecshr is not running from $gtm_dist/gtmsecshrdir or $gtm_dist cannot be determined", 0, 0 },
	...
	{ "SETEXTRENV", "Database files are missing or Instance is frozen; supply the database files, wait for the freeze to lift or define gtm_extract_nocol to extract possibly incorrect collation", 0, 0 },
	...
	{ "SEQNUMSEARCHTIMEOUT", "Timed out trying to find sequence number !@ZQ [0x!16@XQ] in Journal File(s). See above messages for details. Source server exiting", 2, 0 },
	{ "FTOKKEY", "FTOK key 0x!XL", 1, 0 },
	{ "SEMID", "Semaphore id !UL", 1, 0 },
	{ "JNLQIOSALVAGE", "Journal IO lock salvaged", 0, 0 },
	...
	{ "TLSINIT", "Failed to initialize GT.M TLS/SSL library for secure communication", 0, 0 },
	...
	{ "INVLINKTMPDIR", "Value for $gtm_linktmpdir is either not found or not a directory(!AD) - Reverting to default value", 2, 0 },
	...
	{ "REQRLNKCTLRNDWN", "Error accessing relinkctl file !AZ for $ZROUTINES directory !AD. Run MUPIP RUNDOWN -RELINKCTL", 3, 0 },
	...
	{ "INVLOCALE", "Attempt to reset locale to supplied value of $gtm_locale (!AD) failed", 2, 0 },
	...
	{ "JNLNOTALLOWED", "Did not enable journaling on !AD region !AD (!AD)", 6, 0 },
