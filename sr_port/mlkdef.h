...
typedef struct				/* One of these nodes is required for each process which is blocked on a lock */
{
	...
	int4		next;		/* relative pointer to the next mlk_prcblk.  If the entry is in the free
...
{
	int4	value;			/* relative pointer to the shrsub for this node (always present) */
	int4	parent;			/* relative pointer to the parent node (zero if name level) */
	int4	children;		/* relative pointer to the eldest child of this node (zero if none) */
	int4	lsib;			/* relative pointers to the "left sibling" and "right sibling" */
	int4	rsib;			/*    note that each level of the tree is sorted, for faster lookup */
	...
	int4	auxowner;		/* For gt.cm, this contains information on the remote owner of the lock.*/
...
{
	int4		backpointer;	/* relative pointer to mlk_shrblk which owns this entry, so that we can
...
typedef struct	mlk_ctldata_struct	/* this describes the entire shared lock section */
{
	...
	int4		prcfree;			/* relative pointer to the first empty mlk_prcblk */
	int4		prccnt;				/* number of entries in the prcfree chain */
	int4		blkcnt;				/* number of entires in the blkfree chain */
	int4		blkfree;			/* relative pointer to the first free mlk_shrblk.
							 * if zero, the blkcnt must also equal zero */
	int4		blkroot;			/* relative pointer to the first name level mlk_shrblk.
							 * if zero, then there are no locks in this section */
	int4		subbase;			/* relative pointer to the base of the mlk_shrsub area */
	int4		subfree;			/* relative pointer to the first free cell in the shrsub area */
	int4		subtop;				/* relative pointer to the top of the shrsub area */
	int4		reserved;			/* padding to fill out to DW on systems that care */
} mlk_ctldata;
...
#define A2R(X, Y) ((X) = (((sm_uc_ptr_t) (Y)) - ((sm_uc_ptr_t) &(X))))
...
#define MLK_SHRSUB_SIZE(X) (ROUND_UP(sizeof(mlk_shrsub) - 1 + (X)->length, 4))
