...
/* Convert a transaction from jnl version 12 (V4.3-000/1/1A/1B/1C/1D) to 17 (V5.0-000)  */
int jnl_v12tov17(uchar_ptr_t jnl_buff, uint4 *jnl_len, uchar_ptr_t conv_buff, uint4 *conv_len, uint4 conv_bufsiz)
{
		...
		if (0 < (reclen = v12_jnl_record_length((jnl_record *)jb, jlen)))
		{
			if (reclen <= jlen)
			{
				...
				if (IS_SET_KILL_ZKILL(rectype))
				{
					...
					conv_reclen = (unsigned int)(JREC_PREFIX_SIZE + FIXED_UPD_RECLEN + total_key + total_data
						+ JREC_SUFFIX_SIZE);
					conv_reclen = ROUND_UP2(conv_reclen, JNL_REC_START_BNDRY);
				}
				if (IS_COM(rectype))
					conv_reclen = TCOM_RECLEN;
				...
				if (IS_SET_KILL_ZKILL(rectype))
				{
					...
					if (IS_FENCED(rectype))
						jb += (sizeof(token_num) + 8); /* For fenced records, jnl_tid field is 8 bytes */
