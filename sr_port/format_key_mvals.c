...
unsigned char	*format_key_mvals(unsigned char *buff, int size, lvname_info *lvnp)
{
	...
	/* the variable was indirect, and was not found in the frame's l_symtab, so scan the curr_symval to find it */
	if (buff == endbuff)
		FIND_INDIRECT_SYMBOL_NAME(lvnp->start_lvp, buff, endbuff);
	...
	if (cnt > 0 && size > 0)
	{
		*buff++ = '(';
		...
		for (n = 0;  ;  )
		{
			...
			op_fnzwrite(FALSE, keys, &dst);	/* Store ZWRITE representation of keys in "dst" */
