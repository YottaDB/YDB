...
dump(reglist,output,what,detail)	; dump information
	...
	new cmd,dse,debug,error,ydbdist,i,io,isVMS,line,mod,nopipe,region,ver
	...
	set ydbdist=$select(isVMS:"gtm$dist",1:"ydb_dist")
	;
	...
	; DSE command to use
	set dse=$get(%DSEWRAP("alternate"),$ztrnlnm(ydbdist)_"/dse")
...
dseshhdr
	;#!/bin/sh
	;$ydb_dist/dse > dsedump.txt 2>&1 << EOF
	quit
dseshftr
	;EOF
	quit
