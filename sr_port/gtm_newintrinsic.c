...
void gtm_newintrinsic(mval *intrinsic)
{
	...
	{	/* Current (youngest) frame IS an indirect frame.
		...
		*/
		...
		mv_st_ent = (mv_stent *)(top - mvs_size[MVST_MSAV]);
		mv_st_ent->mv_st_type = MVST_MSAV;
		frame_pointer = (stack_frame *)((char *)frame_pointer - mvs_size[MVST_MSAV]);
		...
		{	/* One of the indirect frames has mv_stents associated with it so we have to find
			...
			*/
			fp = (stack_frame *)((char *)fp - mvs_size[MVST_MSAV]);
			mv_chain = (mv_stent *)((char *)mv_chain - mvs_size[MVST_MSAV]);
			mvst_tmp = mv_chain;
