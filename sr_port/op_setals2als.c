...
void op_setals2als(lv_val *srclv, int destindx)
{
	...
	DBGRFCT((stderr, "op_setals2als: hte 0x"lvaddr" being reset from 0x"lvaddr" to 0x"lvaddr"\n",
		 tabent, tabent->value, srclv));
	tabent->value = (void *)srclv;
	INCR_TREFCNT(srclv);
	MARK_ALIAS_ACTIVE(srclv->ptrs.val_ent.parent.sym->symvlvl);		/* This symval has had alias activity */
