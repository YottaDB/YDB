...
GETOUT	; Routine executed just before exiting from GDE. This tries to restore the mumps process context
	...
	; Turn SIGWINCH back off if GDE turned it on (see gde.m - YDB#1247) so it does not outlive GDE
	; in case GDE was invoked from a parent mumps program.
	i $l($g(gdeEntryState("winch"))) u gdeEntryState("io"):nosigwinch
