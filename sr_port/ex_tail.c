		...
		for (i = t->operand; ARRAYTOP(t->operand) > i; i++)
		{
			if (TRIP_REF == i->oprclass)
			{
				...
				if (OCT_BOOL & oc_tab[t0->opcode].octype)
				{
					bx_boollit(t0, depth);
					RETURN_IF_RTS_ERROR;
					if ((OC_COBOOL == t0->opcode) && (OC_LIT == t0->operand[0].oprval.tref->opcode))
					{	/* The above call to "bx_boollit()" did some optimization. In that case,
						 * call "unary_tail()" to remove OC_COMVAL/OC_COBOOL triples and keep just
						 * the OC_LIT triple, if possible. If the removal happens, "opr->oprclass"
						 * would end up as MLIT_REF instead of TRIP_REF hence the below assert.
						 */
						UNARY_TAIL(opr, depth);
						assert((TRIP_REF == opr->oprclass) || (MLIT_REF == opr->oprclass));
					}
				}
				/* Need the below "if" check again (compared to the enclosing "if" check) in the case
				 * "i->oprclass" got changed in the UNARY_TAIL call above due to boolean literal optimization.
				 */
				if (TRIP_REF == i->oprclass)
