...
/* Note: This C function is called by "opp_nequnul_retbool.s" on sr_aarch64 and sr_armv7l.
 * On sr_x86_64, the .s file implements this functionality for performance reasons.
 */
int	op_nequnul_retbool(mval *u)
{
	int	mvtype;
	int	ret;
	...
	if (!MVTYPE_IS_STRING(mvtype))
	{
		...
		else
		{
			 ...
			 * Treat it as "" in that case. Therefore return FALSE (i.e. u'="" is FALSE).
			 */
