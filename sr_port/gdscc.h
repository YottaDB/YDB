...
typedef struct cw_set_element_struct
{
	...
	cache_rec_ptr_t	cr;
	JNL_OFF_T(jnl_freeaddr,1);			/* journal update address */
	...
	short		reference_cnt;			/* Change to free blocks when a bitmap is changed */
	char		level;				/* Block level for newly created blocks	*/
        bool 		done;           		/* Has this update been done already? */
	bool		first_copy;			/* If overlaying same buffer, set if first copy needed */
							/* just an optimisation - avoids copying first few bytes, if anyway
							 * we are just overlaying the new_buff in the same transaction */
	bool		forward_process;		/* Need to process update array from front when doing kills */
	short		t_level;			/* transaction level associated with cw element, for incremental rollback */
