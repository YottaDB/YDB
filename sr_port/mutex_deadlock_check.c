	...
	if (is_replicator || mu_reorg_process)
	{
		...
		if (!dollar_tlevel)
		{
			...
			{	/* grab_lock going for crit on the jnlpool region. gv_cur_region points to the current region of
				 * interest, which better have REPL_ENABLED or REPL_WAS_ENABLED. Assert that. The most common
				 * way this can happen is with the update process which could be adding a history record to the
				 * replication instance file in which case it would have no region of interest (i.e. cs_addrs
				 * could be NULL) but it is also possible when a simpleapi call comes in and is in the process
				 * of opening a database file which notices the jnlpool counter semaphore has overflown.
				 */
				assert((NULL == cs_addrs) || REPL_ALLOWED(cs_addrs));
