/****************************************************************
 *								*
 *	Copyright 2001 Sanchez Computer Associates, Inc.	*
 *								*
 *	This source code contains the intellectual property	*
 *	of its copyright holder(s), and is made available	*
 *	under a license.  If you do not know the terms of	*
 *	the license, please stop and do not read further.	*
 *								*
 ****************************************************************/

#include "mdef.h"
#include "gtm_string.h"
#include "compiler.h"
#include "stringpool.h"
#include "opcode.h"
#include "mdq.h"
#include "advancewindow.h"
#include "pattern.h"
#include "patstr.h"

GBLREF char *lexical_ptr;
GBLREF unsigned char *source_buffer;
GBLREF short int source_column, last_source_column;
GBLREF bool shift_gvrefs;
GBLREF triple *expr_start;

#ifndef BIGENDIAN
GBLDEF unsigned char pat_everything[] = { 0, 2, 0x1f, 1, 0, 0, 0, 0xff, 0x7f, 0, 0, 0xff, 0x7f, 1, 0 }; /* pattern = ".e" */
#else
GBLDEF unsigned char pat_everything[] = { 0, 2, 0x1f, 0, 1, 0, 0, 0x7f, 0xff, 0, 0, 0x7f, 0xff, 0, 1 }; /* pattern = ".e" */
#endif

GBLDEF unsigned char sizeof_pat_everything = sizeof(pat_everything);

int pattern(oprtype *z, bool is_indirect)
{
	mval		retval;
	mstr		instr;
	int		status;
	triple		*oldchain, tmpchain, *ref, *triptr;

	if (is_indirect)
	{
		if (shift_gvrefs)
		{
			dqinit(&tmpchain, exorder);
			oldchain = setcurtchain(&tmpchain);
			if (!indirection(z))
			{
				setcurtchain(oldchain);
				return FALSE;
			}
			ref = newtriple(OC_INDPAT);
			newtriple(OC_GVSAVTARG);
			setcurtchain(oldchain);
			dqadd(expr_start, &tmpchain, exorder);
			expr_start = tmpchain.exorder.bl;
			triptr = newtriple(OC_GVRECTARG);
			triptr->operand[0] = put_tref(expr_start);
		}
		else
		{
			if (!indirection(z))
				return FALSE;
			ref = newtriple(OC_INDPAT);
		}
		ref->operand[0] = *z;
		*z = put_tref(ref);
		return TRUE;
	}
	else
	{
		instr.addr = (char *)&source_buffer[source_column - 1];
		instr.len = strlen(instr.addr);
		status = patstr(&instr, &retval);
		last_source_column = (short int)( instr.addr -  (char *)source_buffer);
		assert(last_source_column);
		if (status)	/* status == syntax error when non-zero */
		{	stx_error(status);
			return FALSE;
		}
		*z = put_lit(&retval);
		lexical_ptr = (char *)&source_buffer[last_source_column - 1];
		advancewindow();
		advancewindow();
		return TRUE;
	}
}


