	...
	for (recstat = 0; recstat < TOT_EXTR_TYPES; recstat++)
	{
		...
		for (rctl = mur_ctl, rctl_top = mur_ctl + murgbl.reg_total; rctl < rctl_top; rctl++)
		{
			...
			{	/* This is not a normal exit of MUPIP JOURNAL. Delete any files that we created.
				...
				{ /* intermediate files are not created and unlinked for DEVNULL */
		...
		{	/* If not a regular file, no file closing message. */
			if (mur_options.extr_fn[recstat] && mur_options.extr_fn_is_regfile[recstat])
