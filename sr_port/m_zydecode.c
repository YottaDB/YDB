...
int m_zydecode(void)
{
	...
	switch (TREF(window_token))
	{
		case TK_IDENT:
			...
			ref = maketriple(OC_ZYDECODE_LVARG);
		...
		case TK_CIRCUMFLEX:
			...
			ref = maketriple(OC_ZYDECODE_GVARG);
		...
		case TK_ATSIGN:
			...
			if (TK_EQUAL != TREF(window_token))
			{
				...
				ref->operand[0] = mopr;
				ref->operand[1] = put_ilit((mint) indir_zydecode);
				return TRUE;
			}
			...
			{	/* save and restore the variable lookup for true left-to-right evaluation */
				...
				ref = maketriple(OC_INDZYDECODE2);
