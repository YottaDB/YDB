...
uint4 jnl_file_open_common(gd_region *reg, off_jnl_t os_file_size)
{
	...
	unsigned char		eof_rec[DISK_BLOCK_SIZE * 2 + ALIGNMENT_SIZE];
	off_jnl_t		adjust;
#if defined(VMS)
	...
	eof_rec_buffer = (unsigned char *)(ROUND_UP2((uintszofptr_t)eof_rec, DISK_BLOCK_SIZE));
	...
	assert(ROUND_UP2(adjust + EOF_RECLEN, DISK_BLOCK_SIZE) <=
		(DISK_BLOCK_SIZE * 2 + ALIGNMENT_SIZE - (eof_rec_buffer - eof_rec)));
	/* Read the journal JRT_EOF at header->end_of_data offset */
	DO_FILE_READ(jpc->channel, header->end_of_data - adjust, eof_rec_buffer,
	...
	{	/* if jb->free does not start at a 512-byte boundary (which is the granularity used by VMS jnl_output_sp() for
		 ...
		 */
		memcpy(&jb->buff[ROUND_DOWN2(jb->free, DISK_BLOCK_SIZE) + jb->buff_off], eof_rec_buffer, adjust);
	}
