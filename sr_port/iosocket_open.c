...
short	iosocket_open(io_log_name *dev, mval *pp, int file_des, mval *mspace, int4 timepar)
{
	...
	{ /* ZFF="non-zero-len-string" specified */
		...
		{ /* Note: the ZFF string originates from the source program, so is in UTF-8 mode or M mode regardless of OCHSET
		   * of this device. ZFF is output on WRITE # command, and MUST contain valid UTF-8 sequence. */
