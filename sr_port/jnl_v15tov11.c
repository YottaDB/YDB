...
int jnl_v15tov11(uchar_ptr_t jnl_buff, uint4 *jnl_len, uchar_ptr_t conv_buff, uint4 *conv_len, uint4 conv_bufsiz)
{
	/* Convert a transaction from jnl version  15 (V.4.4-002) to 11 (V4.2-002) */
   	...
   	while (JREC_PREFIX_SIZE <= jlen)
	{
   		...
   		if (0 != (reclen = prefix->forwptr))
		{
   			if (reclen <= jlen)
			{
				...
				clen_without_sfx = ROUND_UP(V11_JREC_PREFIX_SIZE + v11_jnl_fixed_size[rectype] + des_total_key +
							    total_data, V11_JNL_REC_START_BNDRY);
				...
				if (IS_SET_KILL_ZKILL(rectype))
				{
					memset(cb, 0, V11_JREC_PREFIX_SIZE + V11_MUMPS_NODE_OFFSET); /* unused fields will be 0 */
					*cb = (char)rectype;
					cb += V11_JREC_PREFIX_SIZE + V11_JNL_SEQNO_OFFSET;
					memcpy(cb, jb + V15_JNL_SEQNO_OFFSET, sizeof(seq_num));
					cb += sizeof(seq_num);
					if (IS_FENCED(rectype))
					{
						assert(sizeof(seq_num) == sizeof(token_num));
						/* V15 does not have seperate token field, but
						 * sequence number can be considered as token */
						memcpy(cb, jb + V15_JNL_SEQNO_OFFSET, sizeof(seq_num));
						cb += sizeof(seq_num);
						memset(cb, 0, 8);	/* jnl_tid */
						cb += 8;
					}
					jb +=  FIXED_UPD_RECLEN;
				...
				{
					memset(cb, 0, V11_JREC_PREFIX_SIZE + v11_jnl_fixed_size[rectype]);
					*cb = (char)rectype;
					cb += V11_JREC_PREFIX_SIZE + V11_JNL_SEQNO_OFFSET;
					memcpy(cb, jb + V15_JNL_SEQNO_OFFSET, sizeof(seq_num));
					cb += sizeof(seq_num);
					memcpy(cb, jb + V15_JNL_SEQNO_OFFSET, sizeof(seq_num));	/* token */
					cb += sizeof(token_num);
					memcpy(cb, jb + V15_TCOM_PARTICIPANTS_OFFSET, sizeof(uint4));
					cb += sizeof(uint4) + sizeof(uint4);
				} else
					assert(FALSE);
				nzeros = (cstart + clen_without_sfx - cb);
				if (nzeros > 0)
				{
					assert(V11_JNL_REC_START_BNDRY > nzeros);
					memset(cb, 0, nzeros);
					cb += nzeros;
				}
				jb = jstart + reclen;
				memset(cb, 0, V11_JREC_SUFFIX_SIZE);
