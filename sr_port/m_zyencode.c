...
int m_zyencode(void)
{
	...
	switch (TREF(window_token))
	{
		case TK_IDENT:
			...
			ref = maketriple(OC_ZYENCODE_LVARG);
			ref->operand[0] = put_ilit(ARG1_LCL);
			ref->operand[1] = mopr;
		...
		case TK_CIRCUMFLEX:
			...
			ref = maketriple(OC_ZYENCODE_GVARG);
			ref->operand[0] = put_ilit(ARG1_GBL);
			dqins(tmpchain.exorder.bl, exorder, ref);
			break;
		case TK_ATSIGN:
			if (!indirection(&mopr))
			...
			if (TK_EQUAL != TREF(window_token))
			{
				...
				ref->operand[0] = mopr;
				ref->operand[1] = put_ilit((mint) indir_zyencode);
				return TRUE;
			}
			type = ARG1_LCL | ARG1_GBL;
			...
			{	/* save and restore the variable lookup for true left-to-right evaluation */
				...
				ref = maketriple(OC_INDZYENCODE2);
				ref->operand[0] = control_slot;
			} else
			{	/* quick and dirty old way */
				ref = maketriple(OC_INDZYENCODE);
	...
	switch (TREF(window_token))
	{
		case TK_IDENT:
			if (!lvn(&mopr, OC_M_SRCHINDX, 0))
				return FALSE;
			ref = newtriple(OC_ZYENCODE_LVARG);
			ref->operand[0] = put_ilit(ARG2_LCL);
			ref->operand[1] = mopr;
			break;
		case TK_CIRCUMFLEX:
			if (!gvn())
				return FALSE;
			ref = newtriple(OC_ZYENCODE_GVARG);
			ref->operand[0] = put_ilit(ARG2_GBL);
			break;
		case TK_ATSIGN:
			TREF(temp_subs) = TRUE;
			if (!indirection(&mopr))
