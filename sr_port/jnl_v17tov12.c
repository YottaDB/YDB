...
/* Convert a transaction from jnl version  17 (V5) to 12  (GT.M versions V4.3-000 through V4.3-001E) */
int jnl_v17tov12(uchar_ptr_t jnl_buff, uint4 *jnl_len, uchar_ptr_t conv_buff, uint4 *conv_len, uint4 conv_bufsiz)
{
   			...
   			if (reclen <= jlen)
			{
				...
				if (IS_SET_KILL_ZKILL(rectype))
				{
					...
					*cb = (char)rectype;
					...
					memcpy(cb, jb + V17_JNL_SEQNO_OFFSET, sizeof(seq_num));
					cb += sizeof(seq_num);
				...
				} else if (IS_COM(rectype))
				{
					...
					*cb = (char)rectype;
					...
					memcpy(cb, jb + V17_JNL_SEQNO_OFFSET, sizeof(seq_num));
					cb += sizeof(seq_num);
					memcpy(cb, jb + V17_TCOM_PARTICIPANTS_OFFSET, sizeof(uint4));
