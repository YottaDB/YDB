		...
		while (TRUE)
		{
			...
			if (!prev_catchup && catchup) /* transition from non catchup to catchup */
			{
				repl_log(gtmsource_log_fp, TRUE, TRUE, "Source server entering catchup mode at Seqno : "INT8_FMT" "
					 "Current backlog : "INT8_FMT" Backlog size in journal pool : "INT8_FMT" bytes\n",
			...
			} else if (prev_catchup && !catchup) /* transition from catchup to non catchup */
			{
				repl_log(gtmsource_log_fp, TRUE, TRUE, "Source server returning to regular mode from catchup mode "
					 "at Seqno : "INT8_FMT" Current backlog : "INT8_FMT" Backlog size in journal pool : "
					 INT8_FMT" bytes\n", pre_read_jnl_seqno, backlog_count, backlog_bytes);
			...
			if (0 <= tot_tr_len)
			{
				if (0 < data_len)
				{
					 ...
					 */
					REPL_SEND_LOOP(gtmsource_sock_fd, send_msgp, send_tr_len, UNIX_ONLY(catchup) VMS_ONLY(TRUE),
							&poll_immediate)
					{
						gtmsource_poll_actions(FALSE);
						if (GTMSOURCE_CHANGING_MODE == gtmsource_state)
