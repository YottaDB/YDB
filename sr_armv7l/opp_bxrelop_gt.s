...
/* opp_bxrelop_gt.s */
	...
	.title	opp_bxrelop_gt.s
	.sbttl	opp_bxrelop_gt
...
#	include "debug.si"
	...
	.text
	.extern	op_bxrelop_gt

# Note: op_bxrelop_gt() expects 5 arguments. But only 4 arguments are passed in registers. The 5th argument
# coming into opp_bxrelop_gt.s is passed through the caller's stack. Since we do some stack manipulations locally (`push {fp,lr}`)
# the arguments coming in the caller's stack are not usable when we call op_bxrelop_gt(). Therefore we need to copy them
