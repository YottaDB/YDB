...
/* opp_bxrelop_follow.s */
	...
	.title	opp_bxrelop_follow.s
	.sbttl	opp_bxrelop_follow
...
# Note: op_bxrelop_follow() expects 5 arguments. But only 4 arguments are passed in registers. The 5th argument
# coming into opp_bxrelop_follow.s is passed through the caller's stack. Since we do some stack manipulations locally (`push {fp,lr}`)
# the arguments coming in the caller's stack are not usable when we call op_bxrelop_follow(). Therefore we need to copy them
