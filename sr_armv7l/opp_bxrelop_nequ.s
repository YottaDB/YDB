...
/* opp_bxrelop_nequ.s */
	...
	.title	opp_bxrelop_nequ.s
	.sbttl	opp_bxrelop_nequ
...
#	include "debug.si"
	...
	.text
	.extern	op_bxrelop_nequ

# Note: op_bxrelop_nequ() expects 5 arguments. But only 4 arguments are passed in registers. The 5th argument
# coming into opp_bxrelop_nequ.s is passed through the caller's stack. Since we do some stack manipulations locally (`push {fp,lr}`)
# the arguments coming in the caller's stack are not usable when we call op_bxrelop_nequ(). Therefore we need to copy them
