...
YottaDB<sup>Ⓡ</sup> is a rock solid, lightning fast, and secure database that scales to meet the needs of the very largest, mission critical applications, especially those which need "five nines" availability.
...
YottaDB is a hierarchical key-value database with strongly ACID transactions that does not make you choose between consistency, reliability and performance, even with tens of thousands of concurrent users.

### The Best Database for Systems with Tens of Thousands of Concurrent Processes

The YottaDB database engine runs in-process, on one node and scales vertically. Its transactions have [completely automated retry logic](https://yottadb.com/bid-wars/) that is included out of the box and [optimistic concurrency control](http://daslab.seas.harvard.edu/reading-group/papers/kung.pdf) for ACID  properties, even with tens of thousands of concurrent processes making tens of millions of database accesses, all on a single system.
...
The [Get Started page](https://yottadb.com/product/get-started/) gets you started with YottaDB, on your machine, on our virtual machine, or a Docker container. YottaDB provides pre-built binary distributions for current versions of popular Linux distributions ([Debian](https://www.debian.org/), [Red Hat](https://www.redhat.com/en), [SUSE](https://www.suse.com/), and [Ubuntu](https://ubuntu.com/), as well as selected derivatives), and the [ydbinstall.sh](https://download.yottadb.com/ydbinstall.sh) script downloads and installs the current release for Supported platforms. Details are on the Get Started page.

Refer to the [Release Notes](https://gitlab.com/YottaDB/DB/YDB/-/releases) for each release for a list of the Supported platforms on which we build and test YottaDB binary distributions.
...
`ydbinstall.sh --help` gives you a list of all options.  The script also checks for prerequisite packages and utility programs.
...
```
  ...
  (e.g. for YottaDB r2.06, the default installation path is `/usr/local/lib/yottadb/r206`),
  but can be controlled using the ```--installdir``` option. Run ```./ydbinstall --help``` for a list of options.

  Note that if the ```ydb_icu_version``` environment variable is set to a value other than what `pkg-config --modversion icu-io`
  would return (observed on a SLED 15 or openSUSE Leap system), then the value of that environment variable needs to be preserved across
...
**YottaDB is a registered trademark and wordmark of YottaDB LLC.** If you use, or distribute, a binary version built from unmodified YottaDB source code, you may refer to that software as YottaDB as long as you acknowledge the trademark / wordmark. If you modify the source code in any way, you may not refer to it as YottaDB unless you have explicit written permission to do so from YottaDB LLC.
